...
import { getDisplayDistrictLabel } from "@shared/districtRouting";
...
export default function DTDOApplicationReview() {
  ...
  const isLegacyRC = data?.application?.applicationNumber?.startsWith('LG-HS-') ?? false;
  ...
  const actionMutation = useMutation({
    mutationFn: async ({ action, remarks, inspectionDate, assignedTo }: {
      ...
      if (action === 'reset-revert') {
        endpoint = `/api/dtdo/applications/${id}/reset-revert-count`;
        body = { reason: remarks };
    ...
    onSuccess: (data, variables) => {
      ...
      if (variables.action === 'accept') {
      ...
      } else if (variables.action === 'reset-revert') {
        toast({
          title: "Revert Count Reset",
          description: "Revert counter successfully reset to 0.",
        });
        ...
        toast({
          ...
          description: "Application has been reactivated and moved to Submitted state.",
        });
      } else {
        toast({
          title: "Success",
  ...
  // isLegacyRC moved up for routing logic
      ...
      const context =
        actionType === 'accept'
          ...
          : actionType === 'approve-cancellation'
            ...
            : actionType === 'reject'
              ? 'rejection'
              : actionType === 'reset-revert'
                ? 'resetting revert count'
  ...
  return (
    <>
      ...
      {/* Screen-only Content */}
      <div className="print:hidden container mx-auto p-6 max-w-7xl space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setLocation(isLegacyRC ? "/dtdo/legacy" : "/dtdo/dashboard")}
              ...
              <TabsContent value="property" className="space-y-4 mt-4">
                <Card>
                  ...
                  <CardContent className="grid md:grid-cols-2 gap-4">
                    ...
                    <DetailRow label="District / Tehsil" value={`${getDisplayDistrictLabel(application.district)} • ${application.tehsil ?? "N/A"}`} />
            ...
            {isActionableStatus ? (
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  {/* Reset Revert Count - Only if revert count > 0 */}
                  {(application.revertCount ?? 0) > 0 && (
                    <Button
                      className="w-full"
                      variant="ghost"
                      size="sm"
                      onClick={() => handleAction('reset-revert')}
                      disabled={actionMutation.isPending}
                      data-testid="button-reset-revert"
                    >
                      <RotateCcw className="mr-2 h-3 w-3" />
                      Reset Revert Count ({application.revertCount})
                    </Button>
                  )}
                  {application?.applicationKind === 'cancel_certificate' && (
              ...
              <Card>
                ...
                {applicationStatus === 'rejected' && (
                  <CardContent>
                    <Button
                      className="w-full"
                      variant="default" // Using default (primary) to make it obvious
                    ...
                    <p className="text-xs text-muted-foreground mt-2 text-center">
                      This will move the application back to "Submitted" state and allow one more correction attempt.
                    </p>
                  </CardContent>
                )}
              </Card>
          ...
          <DialogContent className={actionType === 'accept' ? 'max-w-lg' : ''}>
            <DialogHeader>
              <DialogTitle>
                ...
                {actionType === 'reset-revert' && 'Reset Revert Count'}
                {actionType === 'reactivate' && 'Reactivate Application'}
              </DialogTitle>
              <DialogDescription>
                {actionType === 'accept' && (isLegacyRC ? 'Approve and issue certificate for this legacy application immediately (No inspection required).' : 'Schedule the site inspection and assign a Dealing Assistant.')}
                ...
                {actionType === 'reset-revert' && 'This will reset the revert counter to 0. Use this only for valid reasons (e.g. system error).'}
                {actionType === 'reactivate' && 'This will restore the application to "Submitted" status and allow the applicant one more chance to correct it.'}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="remarks">
                  {actionType === 'accept'
                    ? (isLegacyRC ? 'Approval Remarks' : 'Instructions for Inspection Team')
                    : actionType === 'reset-revert'
                ...
                <Textarea
                  ...
                  placeholder={
                    actionType === 'accept'
                      ? (isLegacyRC ? 'Optional instructions or notes...' : 'Share instructions or observations for the inspection team...')
                      : actionType === 'reject'
                        ? 'Please specify the reason for rejection...'
                        : actionType === 'approve-bypass'
                          ? 'Confirm approval without inspection...'
                          : actionType === 'reset-revert'
                            ? 'Explain why the revert count is being reset...'
                            : actionType === 'reactivate'
                              ? 'Explain why you are reactivating this rejected application...'
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={
                  ...
                  (actionType === 'accept' && !isLegacyRC && (!selectedDate || !selectedTime || !assignedDA))
                }
