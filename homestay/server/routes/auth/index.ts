...
const getOtpLoginAvailabilityForUser = async (user: User): Promise<OtpLoginAvailability> => {
  ...
  // OTP login is available for ALL roles when OTP channels are enabled (UI configurable)
...
export function createAuthRouter() {
    ...
    try {
      ...
      req.session.regenerate((err) => {
        ...
        req.session.save(async (saveErr) => {
          if (saveErr) {
            authLog.error("[auth] Failed to save session during registration", saveErr);
            return res.status(500).json({ message: "Registration failed (save error)" });
          }
          // Enforce Single Session (Redundant for new users but good practice)
  ...
  router.post("/login", authRateLimiter, async (req, res) => {
    try {
      ...
      // Enforce Single Session: Invalidation happens after session creation to avoid race conditions
      ...
      req.session.regenerate((err) => {
        ...
        // Explicitly save to ensure session is in DB before response (avoids race with immediate next request)
        req.session.save(async (saveErr) => {
          if (saveErr) {
            authLog.error("[auth] Failed to save session during login", saveErr);
            return res.status(500).json({ message: "Login failed (save error)" });
          }

          // Now that CURRENT session is safe in DB, invalidate OTHERS
          await invalidateExistingSessions(user.id, req.session.id);
  ...
  router.post("/login/verify-otp", authRateLimiter, async (req, res) => {
    try {
      ...
      req.session.regenerate((err) => {
        ...
        req.session.save(async (saveErr) => {
          if (saveErr) {
            authLog.error("[auth] Failed to save session during OTP login", saveErr);
            return res.status(500).json({ message: "Login failed (save error)" });
          }
