...
export default function ExistingOwnerOnboarding() {
  ...
  useEffect(() => {
    ...
    if (shouldRedirectPrimary && !hasRedirected.current) {
    ...
    if (existingApplication && !hasRedirected.current) {
      // Prevent redirecting if we just submitted (handled by mutation) OR if we are in a loop
      // But we must redirect if an application exists.
      // Use a timeout to ensure we don't conflict with other state updates
      hasRedirected.current = true;
