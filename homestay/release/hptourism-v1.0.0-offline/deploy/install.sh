...
create_database() {
    ...
    # Force password update to ensure sync if user already exists
...
fresh_installation() {
    ...
    read -p "Database username [hptourism]: " db_user
    db_user=${db_user:-hptourism}
    ...
    read -p "Backup directory [/opt/backup/homestay]: " backup_path
    backup_path=${backup_path:-/opt/backup/homestay}
...
update_installation() {
    ...
    # Create backup
    # Try to read BACKUP_PATH from .env
    env_backup_path=$(grep "^BACKUP_PATH=" "$install_path/.env" | cut -d'=' -f2)
    backup_base=${env_backup_path:-/opt/backup/homestay}
    ...
    # Ensure binaries are executable for migration
    print_info "Fixing executable permissions..."
...
restore_from_backup() {
    ...
    # Stop PM2
    pm2 stop hptourism-prod 2>/dev/null || true
    pm2 stop hptourism-rc8 2>/dev/null || true
