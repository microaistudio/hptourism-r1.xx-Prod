...
function safeFormatDistance(dateStr: string | undefined): string {
                        ...
                        {settings?.lastBackupAt ? (
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm">
                                    {safeFormatDistance(settings.lastBackupAt)}
                                </span>
                            </div>
                        ) : (
                ...
                <CardContent className="space-y-6">
                    {isEditing ? (
                        <>
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="space-y-4 border-t pt-4 mt-2">
                                    <h3 className="text-sm font-medium">Backup Redundancy</h3>

                                    <div className="flex items-center justify-between">
                                        <div className="space-y-0.5">
                                            <Label>Enable Failover / Fallback</Label>
                                            <p className="text-xs text-muted-foreground">Use a secondary location if the primary fails (e.g., NAS outage)</p>
                                        </div>
                                        <Switch
                                            checked={formData.enableFailover ?? settings?.enableFailover ?? false}
                                            onCheckedChange={(checked) => setFormData({ ...formData, enableFailover: checked })}
                                        />
                                    </div>

                                    {(formData.enableFailover ?? settings?.enableFailover) && (
                                        <div className="space-y-2 pl-2 border-l-2 border-muted ml-1">
                                            <Label>Fallback Directory (Local)</Label>
                                            <Input
                                                value={formData.fallbackDirectory ?? settings?.fallbackDirectory ?? ""}
                                                onChange={(e) => setFormData({ ...formData, fallbackDirectory: e.target.value })}
                                                placeholder="/opt/hptourism/homestay/backups_fallback"
                                            />
                                            <p className="text-xs text-muted-foreground">
                                                Recommended: Use a local folder on the server disk.
                                            </p>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <Label>Include Configuration (.env)</Label>
                                        <Switch
                                            checked={formData.includeEnv ?? settings?.includeEnv ?? true}
                                            onCheckedChange={(checked) => setFormData({ ...formData, includeEnv: checked })}
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2 text-sm">
                            ...
                            {settings?.enableFailover && (
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">Fallback:</span>
                                    <span className="font-mono text-xs" title={settings.fallbackDirectory}>{settings.fallbackDirectory?.split('/').pop() || "configured"}</span>
                                </div>
                            )}
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Include Configuration:</span>
                                <span>{settings?.includeEnv ? "Yes" : "No"}</span>
                            </div>
                        </div>
                    )}
                </CardContent>
