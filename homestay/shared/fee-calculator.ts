...
export interface FeeCalculationInput {
  ...
  discountMode?: 'ADDITIVE' | 'SEQUENTIAL'; // Defaults to SEQUENTIAL if not provided
}
...
export function calculateHomestayFee(input: FeeCalculationInput): FeeBreakdown {
  ...
  // Applied AFTER validity discount (Sequential) or on Base (Additive) based on settings
  let femaleOwnerDiscount = 0;
  if (input.ownerGender === 'female') {
    if (input.discountMode === 'ADDITIVE') {
      // Additive: 5% of original amount (base * years)
      femaleOwnerDiscount = totalBeforeDiscounts * 0.05;
    } else {
      // Sequential (Default): 5% of amount after 3-year discount
