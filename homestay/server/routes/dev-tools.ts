...
const getSystemUserId = async () => {
    // Check by email OR mobile to avoid unique constraint violations
    const email = "simulator.system@hp.gov.in";
    const mobile = "9999999999";
    const existing = await db.select().from(users).where(or(eq(users.email, email), eq(users.mobile, mobile))).limit(1);
...
export function createDevToolsRouter() {
    ...
    // 1. SAFETY MIDDLEWARE (Block in strict production)
