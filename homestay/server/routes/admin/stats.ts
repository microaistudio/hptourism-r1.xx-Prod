...
export function createAdminStatsRouter() {
  ...
  router.get("/dashboard/stats", requireRole("super_admin", "admin", "system_admin"), async (_req, res) => {
    try {
  ...
  router.get("/stats", requireRole("super_admin", "admin", "system_admin"), async (_req, res) => {
    try {
