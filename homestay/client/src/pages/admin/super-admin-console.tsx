...
export default function SuperAdminConsole() {
  ...
  // Fetch current user for headless validation (split-routing fix)
  ...
  const verifyPasswordMutation = useMutation({
    mutationFn: async (password: string) => {
      ...
      // Pass username for headless validation
  ...
  // Handle tab change with password gate for critical tab
  const handleTabChange = (value: string) => {
    if (value === "critical" && !criticalUnlocked) {
  ...
  // Detect hash on mount to switch to the correct tab and scroll
  useEffect(() => {
    const hash = window.location.hash;
    if (!hash) return;

    const criticalSections = ["#reset-zone", "#reset-seeds", "#payment-settings", "#security", "#smoke-tests", "#database", "#test-data"];
    const operationsSections = ["#system-overview", "#communications", "#staff-tools", "#portal-config"];

    const isCritical = criticalSections.some(s => hash.startsWith(s));
    const isOperations = operationsSections.some(s => hash.startsWith(s));

    if (isCritical) {
      // If navigating to critical section, show password dialog if not unlocked
      if (!criticalUnlocked) {
        setPasswordDialog({ open: true, password: "", error: "" });
      } else {
        setActiveTab("critical");
  ...
  // Woman Owner Discount Mode
  const { data: womanDiscountSettings, isLoading: womanDiscountLoading } = useQuery({
    queryKey: ['/api/settings/woman-discount-mode'],
  });

  const updateWomanDiscountModeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Woman owner discount calculation updated.",
      });
    },
    onError: () => {
      toast({
        title: "Update Failed",
        description: "Could not update discount policy.",
        variant: "destructive",
      });
    }
  });
  ...
  // DDO codes for filter dropdown
  const { data: ddoCodesForFilter } = useQuery<{ ddoCodes: { ddoCode: string; district: string; ddoDescription: string }[] }>({
    queryKey: ["/api/admin/reports/ddo-codes"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/admin/reports/ddo-codes");
      return response.json();
    },
  });
  ...
  return (
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="flex-1 space-y-8">
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm pt-2 pb-4 mb-4 border-b">
              <TabsList className="grid w-full grid-cols-2 p-1 bg-muted/50 border h-12 shadow-sm">
                <TabsTrigger value="operations" className="px-6 py-2 flex items-center gap-2">
                  <Zap className="w-4 h-4 text-primary" />
                  Daily Operations
                </TabsTrigger>
                <TabsTrigger value="critical" className="px-6 py-2 flex items-center gap-2 font-semibold text-red-600 data-[state=active]:bg-red-50 data-[state=active]:text-red-700">
                  <ShieldAlert className="w-4 h-4" />
                  System Criticals
                </TabsTrigger>
              </TabsList>
              {criticalUnlocked && activeTab === "critical" && (
                <div className="flex items-center gap-2 mt-2">
                  <Badge variant="outline" className="text-green-600 border-green-300 bg-green-50">
                    🔓 Critical Access Unlocked
                  </Badge>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            <TabsContent value="operations" className="space-y-8 mt-0 outline-none p-0 border-none">
              <section id="system-overview" className="space-y-4">
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    ) : (systemStatus?.instances === undefined || systemStatus?.instances === 0) ? (
                      <div className="text-center p-6 bg-slate-50 dark:bg-slate-900 rounded-lg border border-dashed">
                        <p className="text-muted-foreground text-sm">
                          PM2 is not managing this process.
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">
                          This is expected on development servers running with <code className="bg-slate-200 dark:bg-slate-800 px-1 rounded">tsx</code>.
                        </p>
                      </div>
                    ) : (
            ...
            <TabsContent value="critical" className="space-y-8 mt-0 outline-none p-0 border-none">
              {/* Added Shortcut to System Controls Page */}
              <section id="system-controls" className="space-y-4">
                <Card className="hover:border-red-400 transition-all bg-red-50/20 border-red-200">
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      <ShieldAlert className="w-5 h-5 text-red-600" />
                      <div>
                        <CardTitle className="text-red-900">System Controls & Maintenance</CardTitle>
                        <CardDescription className="text-red-700/80">Manage Maintenance Mode (Lockdown) and suspend Payment Gateway pipelines.</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <Button
                      ...
                      onClick={() => window.location.href = '/admin/system-controls'}
                    >
              ...
              <section id="payment-settings" className="space-y-4">
                ...
                {/* Policy Settings: Woman Owner Discount */}
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold">Woman Owner Discount Logic</h3>
                          <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-300">
                            {womanDiscountSettings?.mode === "ADDITIVE" ? "Additive (15% Total)" : "Sequential (~14.5% Total)"}
                          </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          Define how the 5% Woman Owner discount combines with the 10% Validity discount.
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {womanDiscountSettings?.mode === "ADDITIVE"
                            ? "Both discounts are applied on the base amount (5% + 10% = 15%)."
                            : "Woman discount (5%) is applied AFTER the 10% validity discount is deducted."}
                        </p>
                      </div>
                      <div className="flex gap-2">
                        <Button
                          variant={womanDiscountSettings?.mode === "SEQUENTIAL" || !womanDiscountSettings?.mode ? "default" : "outline"}
                          size="sm"
                          onClick={() => updateWomanDiscountModeMutation.mutate("SEQUENTIAL")}
                          disabled={updateWomanDiscountModeMutation.isPending || womanDiscountSettings?.mode === "SEQUENTIAL" || !womanDiscountSettings?.mode}
                        >
                          Sequential (Default)
                        </Button>
                        <Button
                          variant={womanDiscountSettings?.mode === "ADDITIVE" ? "default" : "outline"}
                          size="sm"
                          onClick={() => updateWomanDiscountModeMutation.mutate("ADDITIVE")}
                          disabled={updateWomanDiscountModeMutation.isPending || womanDiscountSettings?.mode === "ADDITIVE"}
                        >
                          Additive
                        </Button>
                      </div>
                ...
                <Card>
                      ...
                      <div className="flex gap-2">
                        <Select value={himkoshDdoFilter} onValueChange={setHimkoshDdoFilter}>
                          ...
                          <SelectContent>
                            ...
                            {ddoCodesForFilter?.ddoCodes?.map((d) => (
                  ...
                  <CardContent className="space-y-4">
                    {himkoshActivityLoading && <p className="text-muted-foreground">Loading...</p>}
                    {himkoshActivityError && <p className="text-red-600">Error loading transactions.</p>}
                    {!himkoshActivityLoading && !himkoshActivityError && latestTransactions.length === 0 && (
      ...
      {/* Test Payment Mode Confirmation Dialog */}
      <AlertDialog
      ...
      {/* Critical Tab Password Gate Dialog */}
      <Dialog
        ...
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-600">
              ...
              System Criticals Access
            </DialogTitle>
            <DialogDescription>
              Re-enter your password to access critical system controls. These actions can affect the entire system.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="critical-password">Your Password</Label>
              <Input
            ...
            <div className="p-3 bg-red-50 border border-red-200 rounded-md">
              <p className="text-xs text-red-700">
                <strong>Warning:</strong> You are about to access controls that can reset data, modify payment settings, and change security configurations. Only proceed if you are authorized.
              </p>
            </div>
          </div>
          <DialogFooter>
