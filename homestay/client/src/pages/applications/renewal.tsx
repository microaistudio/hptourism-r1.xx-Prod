...
import { DEFAULT_ROOM_CALC_MODE, type WomanDiscountMode, DEFAULT_WOMAN_DISCOUNT_MODE } from "@shared/appSettings";
...
const draftSchema = z.object({
  ...
  guardianRelation: z.enum(["s_o", "d_o", "w_o", "c_o", "father", "husband", "guardian"]).optional(),
...
export default function NewApplication() {
  ...
  const { data: womanDiscountSettings } = useQuery({
    queryKey: ['/api/settings/woman-discount-mode'],
    staleTime: 5 * 60 * 1000,
  });
  const discountMode = (womanDiscountSettings?.mode as WomanDiscountMode) || DEFAULT_WOMAN_DISCOUNT_MODE;
  ...
  const urlRcNumber = searchParams.get("rcNumber");
  ...
  const activeApplicationKind = (activeHydratedApplication?.applicationKind as ApplicationKind | undefined) ?? "renewal";
  ...
  const { data: parentApplication } = useQuery<{ application: HomestayApplication }, Error, HomestayApplication>({
    ...
    enabled: !!parentApplicationId && (activeApplicationKind === 'change_category' || activeApplicationKind === 'delete_rooms'),
    staleTime: 5 * 60 * 1000,
    select: (data) => data.application, // Extract from { application: ... } wrapper
  });

  // Backfill rates from parent application for delete_rooms if missing in draft
  // We must update type2Rows directly, because that drives the UI and the form state via applyType2RowsToForm
  useEffect(() => {
    if (activeApplicationKind === 'delete_rooms' && parentApplication && !isDraftLoading) {
      // Backfill form values first to strictly establish source of truth
      const singleRate = Number(parentApplication.singleBedRoomRate || parentApplication.proposedRoomRate || 0);
      const doubleRate = Number(parentApplication.doubleBedRoomRate || parentApplication.proposedRoomRate || 0);
      const suiteRate = Number(parentApplication.familySuiteRate || parentApplication.proposedRoomRate || 0);

      if (singleRate > 0) {
        form.setValue("singleBedRoomRate", singleRate);
      }
      if (doubleRate > 0) {
        form.setValue("doubleBedRoomRate", doubleRate);
      }
      if (suiteRate > 0) {
        form.setValue("familySuiteRate", suiteRate);
      }

      setType2RowsSafe((currentRows) => {
        let rowsChanged = false;
        const newRows = currentRows.map(row => {
          let parentRate = 0;
          if (row.roomType === 'single') parentRate = Number(parentApplication.singleBedRoomRate || parentApplication.proposedRoomRate || 0);
          else if (row.roomType === 'double') parentRate = Number(parentApplication.doubleBedRoomRate || parentApplication.proposedRoomRate || 0);
          else if (row.roomType === 'suite') parentRate = Number(parentApplication.familySuiteRate || parentApplication.proposedRoomRate || 0);

          if (parentRate > 0 && (!row.customRate || row.customRate === "" || Number(row.customRate) === 0)) {
            rowsChanged = true;
            return { ...row, customRate: parentRate, tariffBucket: rateToBucket(parentRate) ?? "lt3k" };
          }
          return row;
        });
        return rowsChanged ? newRows : currentRows;
      });

      // Also backfill property area if missing (this isn't in type2Rows)
      const currentArea = form.getValues("propertyArea");
      if ((!currentArea || currentArea === 0) && Number(parentApplication.propertyArea || 0) > 0) {
        form.setValue("propertyArea", Number(parentApplication.propertyArea || 0));
        form.setValue("propertyAreaUnit", parentApplication.propertyAreaUnit || "sqm");
        form.setValue("propertyAreaConversionRate", Number(parentApplication.propertyAreaConversionRate || 1));
      }
      console.log("DEBUG Backfill Applied:", { activeApplicationKind });
    } else {
      console.log("DEBUG Backfill Skip:", { activeApplicationKind, hasParent: !!parentApplication, loading: isDraftLoading });
    }
  }, [activeApplicationKind, parentApplication, isDraftLoading]);
  ...
  useEffect(() => {
    ...
    if (activeDraftApplication && !isCorrectionMode && step === 1 && !hasAutoNavigatedToDocuments.current && activeApplicationKind !== 'cancel_certificate' && activeApplicationKind !== 'new_registration') {
  ...
  const hydrateFormFromSource = (source: Partial<HomestayApplication> | DraftForm | null | undefined) => {
    ...
    // Normalize special routing districts (Pangi -> Chamba, Kaza -> Lahaul and Spiti)
    let normalizedRawDistrict = canonicalizeInput(rawDistrictValue);
    if (normalizedRawDistrict.toLowerCase() === "pangi") {
      normalizedRawDistrict = "Chamba";
    } else if (normalizedRawDistrict.toLowerCase().includes("kaza") || normalizedRawDistrict.toLowerCase() === "spiti") {
      normalizedRawDistrict = "Lahaul and Spiti";
    } else if (normalizedRawDistrict.toLowerCase() === "lahaul") {
      normalizedRawDistrict = "Lahaul and Spiti";
    }

    const districtValue = findCanonicalMatch(
      normalizedRawDistrict,
      HP_DISTRICTS,
    );
    ...
    form.reset({
      ...
      // guardianRelation is set explicitly below to ensure persistence wins over defaults
      guardianRelation: (() => {
        const raw = ((source as any).guardianRelation || (source as any).guardian_relation || "").toLowerCase();
        if (["s_o", "d_o", "w_o", "c_o"].includes(raw)) return raw;
        // Return raw legacy value (e.g. "father") so validation flags it as invalid
        if (raw) return raw;
        return defaults.guardianRelation ?? "";
      })(),
      ...
      documentsSource.forEach((doc: any) => {
        ...
        switch (doc.documentType) {
          ...
          case "legacy_certificate":
            docs.existingCertificate.push(base);
            break;
  ...
  const buildDocumentsPayload = () => {
    ...
    return [
      ...
      ...normalize(uploadedDocuments.existingCertificate, "legacy_certificate"),
  ...
  const missingSingleRate = !isDeleteRooms && singleBedRooms > 0 && singleBedRoomRate <= 0;
  const missingDoubleRate = !isDeleteRooms && doubleBedRooms > 0 && doubleBedRoomRate <= 0;
  const missingSuiteRate = !isDeleteRooms && familySuites > 0 && familySuiteRate <= 0;
  ...
  const fireSafetyDetailsMissing = selectedAmenities.fireSafety && (form.watch("fireEquipmentDetails")?.trim().length || 0) < 10;
  const propertyAreaValue = form.watch("propertyArea");
  const propertyAreaMissing = !isDeleteRooms && (!propertyAreaValue || Number(propertyAreaValue) <= 0);
  ...
  console.log("Validation Debug:", {
    ...
    propertyAreaMissing,
    fireEquipmentDetails: form.watch("fireEquipmentDetails"),
    fireSafetyDetailsMissing,
    selectedFireSafety: selectedAmenities.fireSafety,
  ...
  const step1FieldsMissing = !propertyName?.trim() || !district?.trim() || !HP_DISTRICTS.includes(district) || !address?.trim() || !locationType || !pincodeIsValid || tehsilMissing || gramPanchayatMissing;

  // Step 2 required field check
  const guardianRelation = form.watch("guardianRelation");
  const step2FieldsMissing = !guardianRelation || !["s_o", "d_o", "w_o", "c_o"].includes(guardianRelation.toLowerCase()) || !guardianName?.trim() || guardianName.trim().length < 3;
  ...
  const validationErrorReason = step === 1 && step1FieldsMissing ? "Step 1 Missing" :
    step === 2 && propertyOwnership === "leased" ? "Lease Check" :
      step === 2 && step2FieldsMissing ? "Step 2 Missing" :
        step === 3 && isLeaseBlocked ? "Lease Blocked" :
          step === 3 && roomLimitExceeded ? "Room Limit (>6)" :
            step === 3 && bedLimitExceeded ? "Bed Limit (>15)" :
              step === 3 && bathroomsBelowRooms ? "Bathrooms < Rooms" :
                step === 3 && propertyAreaMissing ? "Area Missing" :
                  step === 3 && missingTariffs ? "Tariff Missing" :
                    step === 3 && totalRooms === 0 ? "No Rooms" :
                      step === 3 && gstinBlocking ? "GSTIN Block" :
                        step === 3 && !syncAttachedBaths ? "Sync Baths" :
                          step === 3 && roomGuardrailsFailed ? "Guardrails Check" :
                            step === 4 && (!allMandatoryChecked || roomGuardrailsFailed) ? "Step 4 Incomplete" : "";
  ...
  // Auto-set validity for Add Rooms based on existing license expiry
  useEffect(() => {
    if (activeApplicationKind === 'add_rooms' && inheritedCertificateExpiry) {
      const expiry = new Date(inheritedCertificateExpiry);
      const now = new Date();
      const monthsRemaining = differenceInMonths(expiry, now);

      // User Rule: < 12 months -> 1 year fee. > 12 months -> 3 year fee.
      const targetValidity = monthsRemaining >= 12 ? "3" : "1";

      if (certificateValidityYears !== targetValidity) {
        // Only update if different to avoid loop
        form.setValue("certificateValidityYears", targetValidity, {
          shouldDirty: true,
          shouldValidate: true
        });

        toast({
          title: "Fee Calculation Updated",
          description: `Validity aligns with your existing license (expires ${expiry.toLocaleDateString()}). Fee applied for ${targetValidity === "3" ? "3 years" : "1 year"}.`,
        });
      }
    }
  }, [activeApplicationKind, inheritedCertificateExpiry, certificateValidityYears, form]);
  ...
  const saveDraftMutation = useMutation({
    mutationFn: async () => {
      ...
      console.log("[saveDraft] rawFormData.guardianRelation:", rawFormData.guardianRelation);

      // Validate with relaxed draft schema (all fields optional)
      const validatedData = draftSchema.parse(rawFormData);
      console.log("[saveDraft] validatedData.guardianRelation:", validatedData.guardianRelation);
      ...
      if (true) { // Always enter this block if we passed the above check or didn't return
        const response = await fetch("/api/applications/draft", {
          ...
          body: JSON.stringify({
            ...
            // Include rc details from URL so the backend can backfill info if necessary
            parentCertificateNumber: urlRcNumber || undefined,
          }),
        });
  ...
  const submitApplicationMutation = useMutation({
    mutationFn: async (formData: ApplicationForm) => {
      ...
      const payload = {
        ...
        applicationKind: "renewal",
        parentCertificateNumber: activeHydratedApplication?.parentCertificateNumber || urlRcNumber || undefined,
      };
  ...
  const onSubmit = async (data: ApplicationForm) => {
    ...
    if (isUpfrontPayment && !isCorrectionMode && activeApplicationKind !== 'delete_rooms' && activeApplicationKind !== 'cancel_certificate') {
      ...
      // For upfront payment, we ALWAYS save as draft first to ensure latest changes (validity, fees) are persisted
      try {
        toast({
          title: "Saving Application",
          description: "Saving your application details before redirecting to payment...",
        });

        // Use save draft mutation to update backend
        const savedResult = await saveDraftMutation.mutateAsync(data);
        const targetId = savedResult?.application?.id || draftId;
        ...
        if (targetId) {
          ...
          setLocation(`/applications/${targetId}/payment-himkosh?upfront=true`);
        } else {
          console.error("No application ID returned after save");
          toast({ title: "Error", description: "Could not proceed to payment", variant: "destructive" });
        }
      } catch (error) {
  ...
  const nextStep = async () => {
    ...
    if (step === 3) {
      ...
      const singleRate = Number(form.getValues("singleBedRoomRate") || form.getValues("proposedRoomRate") || 0);
      const doubleRate = Number(form.getValues("doubleBedRoomRate") || form.getValues("proposedRoomRate") || 0);
      const suiteRate = Number(form.getValues("familySuiteRate") || form.getValues("proposedRoomRate") || 0);

      if (singleRooms > 0 && singleRate <= 0 && activeApplicationKind !== 'delete_rooms') {
        toast({
      ...
      if (doubleRooms > 0 && doubleRate <= 0 && activeApplicationKind !== 'delete_rooms') {
        toast({
          title: "Room rate required",
      ...
      if (suiteRooms > 0 && suiteRate <= 0 && activeApplicationKind !== 'delete_rooms') {
        toast({
          title: "Room rate required",
    ...
    if (step === 5) {
      ...
      if (!skipDocValidation) {
        const missingDocs = [];
        if (activeApplicationKind === 'renewal') {
          if (!uploadedDocuments.existingCertificate || uploadedDocuments.existingCertificate.length === 0) missingDocs.push("Existing RC Certificate");
        } else {
  ...
  const combinedFormData = {
    ...
    existingCertificate: uploadedDocuments.existingCertificate,
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {step === 3 && (
              <Step3RoomsCategory
                ...
                originalRates={parentApplication ? {
                  single: Number(parentApplication.singleBedRoomRate || parentApplication.proposedRoomRate || 0),
                  double: Number(parentApplication.doubleBedRoomRate || parentApplication.proposedRoomRate || 0),
                  suite: Number(parentApplication.familySuiteRate || parentApplication.proposedRoomRate || 0)
                } : undefined}
                fixedQuantities={activeApplicationKind === 'delete_rooms' && requestedRooms ? {
                  single: requestedRooms.single,
                  double: requestedRooms.double,
                  suite: requestedRooms.family
                } : undefined}
                type2Rows={type2Rows}
...

            {
              step === 5 && (
                <Step5Documents
                  ...
                  correctionNotes={activeCorrectionApplication?.clarificationRequested || activeCorrectionApplication?.rejectionReason || undefined}
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (!isValid) {
                      ...
                      // Find the first step that has a validation error
                      const errorKeys = Object.keys(errors);

                      // Check known required fields from STEP_CONFIG
                      const stepWithErrors = STEP_CONFIG.find(step =>
                        step.requiredFields.some(field => errorKeys.includes(field))
                      );

                      if (stepWithErrors) {
                        setStep(stepWithErrors.id);
                        toast({
                          title: `Validation Error in Step ${stepWithErrors.id}`,
                          description: `Redirecting to ${stepWithErrors.label}. Please check highlighted fields.`,
                          variant: "destructive",
                        });
                        return;
                      }
