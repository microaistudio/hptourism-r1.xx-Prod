...
export function createAdminDevopsRouter() {
  ...
  router.post("/reset/granular", requireRole("super_admin"), async (req, res) => {
    try {
      ...
      // Auto-detect dev/test environments
  ...
  // Use dynamic process name injected by PM2, with fallback for local dev
  ...
  router.get("/system/status", requireRole("super_admin"), async (req, res) => {
    exec('pm2 jlist', (error: any, stdout: string, stderr: string) => {
      ...
      try {
        ...
        // Filter to exact app name match for this environment
        const appProcesses = processes.filter((p: any) =>
          p.name === PM2_APP_NAME
        );
  ...
  router.post("/system/scale", requireRole("super_admin"), async (req, res) => {
    ...
    // Use the defined app name for this environment
    const command = `pm2 scale ${PM2_APP_NAME} ${count}`;
