    ...
    return (
        <Tabs defaultValue="mandatory" className="w-full space-y-6">
            <TabsList className="grid w-full grid-cols-2 p-1 bg-slate-100/80 rounded-xl">
                <TabsTrigger
                    value="mandatory"
                    className="group data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-slate-900 py-3 rounded-lg transition-all duration-300 font-bold flex gap-2 border border-transparent data-[state=active]:border-slate-200"
                >
                    <div className={`p-1 rounded-full ${allMandatoryChecked ? 'bg-slate-100 text-slate-600' : 'bg-slate-100 text-slate-500 group-data-[state=active]:bg-slate-800 group-data-[state=active]:text-white'}`}>
                        {allMandatoryChecked ? <CheckCircle2 className="w-4 h-4" /> : <Shield className="w-4 h-4" />}
                    </div>
                    Mandatory Checklist
                    {allMandatoryChecked && <Badge variant="outline" className="ml-2 bg-slate-100 text-slate-700 border-slate-200">Complete</Badge>}
                </TabsTrigger>
                <TabsTrigger
                    value="optional"
                    className="group data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-slate-900 py-3 rounded-lg transition-all duration-300 font-bold flex gap-2 border border-transparent data-[state=active]:border-slate-200"
                >
                    <div className="p-1 rounded-full bg-slate-100 text-slate-500 group-data-[state=active]:bg-slate-800 group-data-[state=active]:text-white">
            ...
            <TabsContent value="mandatory" className="animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
                <Card className={`shadow-lg overflow-hidden rounded-xl border transition-colors duration-500 ${allMandatoryChecked ? 'border-slate-300' : 'border-slate-200'}`}>
                    <CardContent className="p-0">
                        {/* Section 3: Mandatory Checklist */}
                        <div className="border-b transition-colors duration-500">
                            <div className={`bg-gradient-to-r px-6 py-5 text-white transition-all duration-500 from-slate-800 to-slate-700`}>
                            ...
                            <div className="p-6 space-y-6 bg-slate-50/50">
                                ...
                                {/* Auto-verified items - Clean Card Layout */}
                                <div className="bg-slate-50 rounded-xl p-5 border border-slate-200 shadow-sm">
                                    <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
                                        ...
                                        Auto-Verified Items
                                        <span className="text-xs font-normal text-slate-500 ml-auto">Verified automatically from your application</span>
                                    </h3>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {ANNEXURE_III_MANDATORY.filter(item => item.autoCheck).map(item => (
                                            <div
                                                key={item.id}
                                                className="flex items-center gap-3 px-4 py-4 rounded-xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
                                            >
                                                <div className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0 shadow-sm">
                                                    <Check className="w-4 h-4 text-slate-700" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <span className="text-sm text-gray-800 font-medium leading-tight block">{item.label.split('(')[0].trim()}</span>
                                                </div>
                                                <Badge className="shrink-0 text-[10px] bg-slate-100 text-slate-700 border-slate-200 font-semibold">AUTO</Badge>
                                            </div>
                                        ))}
                                    </div>
                                </div>

                                {/* Manual items - Clean Section */}
                                <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                                    <div className="flex items-center justify-between mb-5">
                                        <h3 className="text-sm font-bold text-slate-900 flex items-center gap-3">
                                            <div className={`w-6 h-6 rounded-full flex items-center justify-center ${manualCheckedCount === manualItemsCount ? 'bg-slate-800' : 'bg-slate-200'}`}>
                                                {manualCheckedCount === manualItemsCount
                                                    ? <Check className="w-4 h-4 text-white" />
                                                    : <span className="text-xs text-slate-700 font-bold">{manualCheckedCount}</span>
                                                }
                                            </div>
                                            <span>Facilities to Confirm</span>
                                            <Badge variant="outline" className={`ml-2 ${manualCheckedCount === manualItemsCount ? 'bg-slate-100 text-slate-700 border-slate-200' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                                                {manualCheckedCount}/{manualItemsCount}
                                            </Badge>
                                        </h3>
                                        <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            className="text-xs h-8 px-4 border-slate-200 hover:bg-slate-100 hover:border-slate-300 text-slate-700 font-medium"
                                            onClick={() => {
                                            ...
                                            return (
                                                <label
                                                    ...
                                                    className={`group relative flex items-start gap-4 p-4 rounded-xl border transition-all duration-200 cursor-pointer ${isChecked
                                                        ? "border-slate-600 bg-slate-50 shadow-sm ring-1 ring-slate-600"
                                                        : "border-gray-200 bg-white hover:border-slate-300 hover:bg-slate-50/50 hover:shadow-sm"
                                                        }`}
                                                >
                                                    <Checkbox
                                                        ...
                                                        className={`mt-0.5 h-5 w-5 rounded transition-all duration-300 ${isChecked ? 'data-[state=checked]:bg-slate-800 data-[state=checked]:border-slate-800' : 'border-gray-300'}`}
                                                    />
                                                    <div className="flex-1">
                                                        <span className={`text-sm font-medium leading-relaxed transition-colors ${isChecked ? 'text-slate-900' : 'text-gray-700 group-hover:text-gray-900'}`}>
                                                            {item.label}
                                                        </span>
                                                    </div>
                                                    {isChecked && (
                                ...
                                return (
                                    <label
                                        ...
                                        className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 group relative select-none ${isChecked
                                            ? 'bg-slate-50 border-slate-600 shadow-sm'
                                    ...
                                    >
                                        <Checkbox
                                            ...
                                            className={`mt-0.5 transition-all duration-300 ${isChecked ? 'data-[state=checked]:bg-slate-800 border-slate-800' : 'border-slate-300'}`}
                                        />
                                        <div className="flex-1 space-y-1">
                                            <span className={`text-sm font-medium transition-colors duration-200 ${isChecked ? 'text-slate-900' : 'text-slate-600 group-hover:text-slate-900'}`}>
                                                {facility.label}
                                            </span>
                                        </div>
                                    </label>
