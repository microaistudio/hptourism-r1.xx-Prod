...
export const OWNER_CREDS: UserCredentials = {
    ...
    password: process.env.SMOKE_OWNER_PASSWORD || "Test@123",
};

export const OWNER2_CREDS: UserCredentials = {
    mobile: "6666666611",
    password: process.env.SMOKE_OWNER_PASSWORD || "Test@123",
};
...
async function cleanupTestData() {
    ...
    const mobiles = [OWNER_CREDS.mobile, OWNER2_CREDS.mobile];

    for (const ownerMobile of mobiles) {
        try {
            const cleanupSQL = `
...
async function cleanupServiceRequests(mobile: string) {
    console.log(`🧹 Cleaning up service requests for ${mobile}...`);
    try {
        const cleanupSQL = `
            DELETE FROM homestay_applications 
            WHERE user_id = (SELECT id FROM users WHERE mobile = '${mobile}') 
            AND application_kind != 'new_registration';
        `;
        execSync(`source .env && psql "$DATABASE_URL" -c "${cleanupSQL}"`, {
            cwd: process.cwd(),
            shell: "/bin/bash",
        ...
        console.log("✅ Service requests cleaned up.");
    } catch (e) {
        console.log("⚠️ Cleanup warning:", (e as Error).message);
    }
}
...
async function setupTestUsers() {
    ...
    // Known hash for "Test@123" ($2b$10$OEnfuI/apZs4aHXm9fq0UuRJzXvNC8nWeYzsR3bhnQaIUigh7Rq2K)
    // We escape $ signs with backslash for bash execution: \$2b\$10\$...
    const HASH_TEST_123 = "\\$2b\\$10\\$OEnfuI/apZs4aHXm9fq0UuRJzXvNC8nWeYzsR3bhnQaIUigh7Rq2K";

    const owners = [
        { ...OWNER_CREDS, role: "property_owner" },
        { ...OWNER2_CREDS, role: "property_owner" }
    ];

    for (const user of owners) {
        const upsertSQL = `
            INSERT INTO users (id, mobile, password, full_name, role, is_active, district)
            VALUES (
                gen_random_uuid(), 
                '${user.mobile}', 
                '${HASH_TEST_123}', 
        ...
        try {
            execSync(`source .env && psql "$DATABASE_URL" -c "${upsertSQL}"`, {
                cwd: process.cwd(),
                shell: "/bin/bash",
                stdio: "pipe", // Hide output unless error
            });
        } catch (e) {
            console.error(`⚠️ Failed to setup user ${user.mobile}:`, (e as Error).message);
        }
    }
...
export async function login(credentials: UserCredentials): Promise<SessionCookie[]> {
    const response = await fetch(`${BASE_URL}/api/auth/login`, {
        ...
        headers: {
            ...
            "Host": "hptourism.osipl.dev", // Bypass Captcha
    ...
    console.log(`Debug: Login cookies for ${credentials.mobile}: `, rawCookies);
...
async function verifyPaymentInitiation(cookies: SessionCookie[], appId: string) {
    console.log("   💸 Verifying Payment Initiation...");

    // 1. Transition to payment pending (if not already)
    // Force status to allow payment initiation
    const forceStatusSQL = `UPDATE homestay_applications SET status = 'payment_pending', total_fee = 18000 WHERE id = '${appId}';`;
    execSync(`source .env && psql "$DATABASE_URL" -c "${forceStatusSQL}"`, { cwd: process.cwd(), shell: "/bin/bash", stdio: "ignore" });

    // 2. Call Initiate API
    const response = await apiCall(cookies, "POST", "/api/himkosh/initiate", { applicationId: appId });

    // Log response for debugging
    if (!response.success) {
        console.log("   ❌ Payment Init Failed Response:", JSON.stringify(response));
    }

    if (!response.success || !response.encdata || !response.checksum) {
        throw new Error("Payment initiation failed: Missing encdata or checksum");
    }
    console.log("   ✅ Payment Initiation API success (EncData + Checksum received)");
}
...
async function ownerResubmitCorrection(cookies: SessionCookie[], appId: string) {
    // Note: Server uses PATCH /api/applications/:id for corrections
    // And automatically transitions status to 'under_scrutiny'
    return await apiCall(cookies, "PATCH", `/api/applications/${appId}`, {
        // Send a dummy update or just minimal data needed
        ownerName: "Smoke Owner (Corrected)",
    });
}
...
async function runEnhancedSmokeTest() {
    ...
    const owner2Cookies = await login(OWNER2_CREDS);
    ...
    /*
        // ========== WORKFLOW 1-4 (Disabled for Single Session Verification) ==========
        /*
        ...
        const workflow1: WorkflowReport = {
            ...
            steps: [
                ...
                { name: "Verify: Payment Init", status: "pending" },
        ...
        try {
            ...
            // NEW: Verify Payment Initiation (Regression test for encryption/checksum)
            // Uses ownerCookies to simulate owner clicking "Pay"
            await runStep("Verify: Payment Init", () => verifyPaymentInitiation(ownerCookies, app1.id), workflow1, 5);
    
            await runStep("DTDO: Approve Application", () => dtdoApprove(dtdoCookies, app1.id), workflow1, 6);
        ...
        const workflow2: WorkflowReport = {
            ...
            steps: [
                ...
                // { name: "DA: Forward to DTDO (Post-Correction)", status: "pending" }, // Resubmission goes directly to DTDO
        ...
        try {
            const app2 = await runStep(
                "Owner: Submit New Application",
                () => createNewApplication(owner2Cookies, {
                    district: "Shimla",
                    category: "silver",
                    rooms: 2,
            ...
            // Insert mock documents for App 2
            await insertMockDocuments(app2.id);
            ...
            await runStep("Owner: Resubmit Correction", () => ownerResubmitCorrection(owner2Cookies, app2.id), workflow2, 3);
            // await runStep("DA: Forward to DTDO (Post-Correction)", () => daForwardToDTDO(daCookies, app2.id), workflow2, 4);
            await runStep("DTDO: Accept", () => dtdoAccept(dtdoCookies, app2.id), workflow2, 4);
            await runStep("DTDO: Approve", () => dtdoApprove(dtdoCookies, app2.id), workflow2, 5);
        ...
        if (baseApprovedAppId) {
            ...
            // Cleanup service request so W4 can proceed
            await cleanupServiceRequests(OWNER_CREDS.mobile);
        }
    ...
    // ========== WORKFLOW 5: Single Concurrent Session ==========
    console.log("\n🚀 Starting workflow: 5. Single Concurrent Session Policy");
    const workflow5: WorkflowReport = {
        id: "5-single-session",
        workflow: "5. Single Concurrent Session Policy",
        steps: [
            { name: "Login Session 1", status: "pending" },
            { name: "Login Session 2 (New Device)", status: "pending" },
            { name: "Verify Session 1 Invalidated", status: "pending" },
            { name: "Verify Session 2 Active", status: "pending" },
        ],
    };
    const wf5Start = Date.now();
    try {
        // 1. Login Session 1
        let cookies1: SessionCookie[] = [];
        await runStep("Login Session 1", async () => {
            cookies1 = await login(OWNER_CREDS);
            if (!cookies1.length) throw new Error("Session 1 login failed");
        }, workflow5, 0);

        // 2. Login Session 2
        let cookies2: SessionCookie[] = [];
        await runStep("Login Session 2 (New Device)", async () => {
            cookies2 = await login(OWNER_CREDS);
            if (!cookies2.length) throw new Error("Session 2 login failed");
        }, workflow5, 1);

        // 3. Verify Session 1 is Invalid
        await runStep("Verify Session 1 Invalidated", async () => {
            const check = await fetch(`${BASE_URL}/api/auth/current-user`, {
                headers: {
            ...
            // If 200 OK but HTML, it means invalid session (login page)
            if (check.ok) {
                const contentType = check.headers.get("content-type");
                if (contentType && contentType.includes("application/json")) {
                    const user = await check.json();
                    throw new Error(`Session 1 is still active! User: ${JSON.stringify(user)}`);
                } else {
                    const text = await check.text();
                    if (text.includes("<!DOCTYPE") || text.includes("<html")) {
                        // Success: Session 1 is invalid
                        return;
                    }
                }
            }

            if (check.status === 401 || check.status === 403 || check.status === 302) {
                return;
            }
        }, workflow5, 2);

        // 4. Verify Session 2 is Active
        await runStep("Verify Session 2 Active", async () => {
            const check = await fetch(`${BASE_URL}/api/auth/current-user`, {
                headers: {
            ...
            if (!check.ok) {
                throw new Error(`Session 2 should be active but got ${check.status}`);
            }
            const user = await check.json() as { mobile: string };
            console.log(`   ✅ Session 2 active for user: ${user.mobile}`);
        }, workflow5, 3);

        console.log(`✅ Workflow "5. Single Concurrent Session" completed`);
    } catch (e) {
        console.log(`❌ Workflow "5. Single Concurrent Session" failed:`, e);
        workflow5.error = (e as Error).message;
    }
    workflow5.duration = Date.now() - wf5Start;
    reports.push(workflow5);
