...
// Helper to mask virtual routing districts to legal district names
...
// Helper to check if app is legacy/existing RC
const isLegacy = (app: ApplicationWithOwner) => isLegacyApplication(app as unknown as HomestayApplication);

export default function DTDODashboard() {
  const [location] = useLocation();
  const searchParams = new URLSearchParams(window.location.search);
  const currentView = searchParams.get("view");
  const isLegacyView = currentView === "legacy";
  ...
  const correctionQueueStatuses = useMemo(
    () => new Set<string>(["reverted_by_dtdo", "objection_raised"]),
    [],
  );
  ...
  const forwardedByDA = useMemo(
    () => viewApplications.filter((app) => app.status === "forwarded_to_dtdo"),
    [viewApplications],
  );
  ...
  const pendingCorrections = useMemo(
    ...
    [viewApplications, needsCorrectionHandling],
  );
  ...
  const inspectionPending = useMemo(
    () => viewApplications.filter((app) => app.status === "inspection_scheduled"),
    [viewApplications],
  );
  const inspectionReports = useMemo(
    () => viewApplications.filter((app) => app.status === "inspection_under_review"),
    [viewApplications],
  );
  const objectionQueue = useMemo(
    () => viewApplications.filter((app) => correctionQueueStatuses.has(app.status)),
    [viewApplications, correctionQueueStatuses],
  );
  ...
  const stageConfigs = useMemo<StageConfig[]>(
    () => [
      ...
      },
      {
        ...
        description: "Manage returned files and owner responses.",
        icon: AlertCircle,
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        ...
                        <div className="text-xs text-muted-foreground">{getDisplayDistrictLabel(app.district)}</div>
                      </td>
                      <td className="p-4">
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground mt-1">
                ...
                <span className="hidden sm:inline">{getDisplayDistrictLabel(application.district)}</span>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            {isLegacyView ? "Existing RC Queue" : "DTDO Dashboard"}
          </h1>
          <p className="text-muted-foreground mt-2">
            {currentUser?.district ? `District: ${getDisplayDistrictLabel(currentUser.district)}` : "Review and process homestay applications"}
          </p>
        </div>
        <Button
          variant="outline"
          ...
          return (
            ...
            >
              ...
              {stage.key === "completed" ? (
                ...
                <p className="text-xs text-muted-foreground line-clamp-1 mb-2">{stage.description}</p>
              )}

              <div className="mt-auto flex flex-col gap-1 w-full text-[11px]">
          ...
          {activeStageConfig.pills.map((pill) => {
            ...
            return (
              ...
              >
                ...
                <span className="rounded-full bg-muted px-2 py-0.5 text-xs font-semibold text-foreground/70">{pill.count}</span>
              ...
              <div className="space-y-3">
                {activePillConfig.applications.map((application) => (
                  <QueueCard
                    ...
                    actionLabel={
                      typeof activePillConfig.actionLabel === "function"
                        ? activePillConfig.actionLabel(application)
                        : activePillConfig.actionLabel
                    }
                    actionHint={
                      typeof activePillConfig.actionHint === "function"
                        ? activePillConfig.actionHint(application)
                        : activePillConfig.actionHint
                    }
