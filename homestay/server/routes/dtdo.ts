...
export function createDtdoRouter() {
    ...
    router.post("/applications/:id/accept", requireRole('district_tourism_officer', 'district_officer'), async (req, res) => {
        try {
            ...
            // EXCEPTION: Simple Service Requests (Delete Rooms, Cancel Cert) or Explicit Deletions
            // Skip inspection -> Approve immediately
            const isServiceRequest =
                application.applicationKind === 'delete_rooms' ||
                application.applicationKind === 'cancel_certificate' ||
                (application.serviceContext?.requestedDeletions && application.serviceContext.requestedDeletions.length > 0) ||
                (application.serviceContext?.legacyOnboarding === true);
    ...
    router.get("/available-das", requireRole('district_tourism_officer', 'district_officer'), async (req, res) => {
        try {
            ...
            const manifestEntries = getDistrictStaffManifest().filter((entry) => {
                // Return entries where the entry's district is covered by the current user
                // e.g. if user covers Hamirpur+Una, return manifest entries for Hamirpur AND Una
    ...
    router.post("/schedule-inspection", requireRole('district_tourism_officer', 'district_officer'), async (req, res) => {
        try {
            ...
            const isDaCovered = assignedDaUser && assignedDaUser.district ?
                coveredDistricts.some(d => districtsMatch(d, assignedDaUser.district)) : false;
    ...
    // POST /applications/:id/reset-revert-count - Manually reset revert count (DTDO only)
    router.post("/applications/:id/reset-revert-count", requireRole('district_tourism_officer', 'district_officer'), async (req, res) => {
        try {
            const { id } = req.params;
            const { reason } = req.body;

            if (!reason) {
                return res.status(400).json({ message: "Reason is required for resetting revert count" });
            }
            ...
            // Check if user has access (district check)
            const userId = req.session.userId!;
            const user = await storage.getUser(userId);
            ...
            await logApplicationAction({
                applicationId: id,
                actorId: userId,
                action: "revert_limit_reset",
                previousStatus: application.status,
                newStatus: application.status,
                feedback: `Revert limit reset. Reason: ${reason}. Previous count: ${previousRevertCount}.`,
            });

            res.json({ message: "Revert count reset successfully", revertCount: 0 });

        } catch (error) {
            console.error("Error resetting revert count:", error);
            res.status(500).json({ message: "Failed to reset revert count" });
        }
    });
