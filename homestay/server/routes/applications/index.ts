...
export function createApplicationsRouter() {
  ...
  router.get("/:id", requireAuth, async (req, res) => {
    try {
      ...
      const CORRECTION_STATUSES = ['reverted_to_applicant', 'sent_back_for_corrections', 'reverted_by_dtdo'];
      if (CORRECTION_STATUSES.includes(application.status)) {
        const tableDocuments = await storage.getDocumentsByApplication(req.params.id);
        if (tableDocuments.length > 0) {
          // Build maps for document lookup
          const verificationByPath = new Map<string, { verificationStatus: string | null; verificationNotes: string | null; documentType: string | null }>();
          const verificationByType = new Map<string, { verificationStatus: string | null; verificationNotes: string | null }>();
          ...
          for (const doc of tableDocuments) {
            ...
            if (doc.filePath) {
              verificationByPath.set(doc.filePath, { ...data, documentType: doc.documentType });
            }
            // Also index by documentType for fallback matching
            if (doc.documentType) {
              verificationByType.set(doc.documentType, data);
            }
          }

          // Merge verification data into application's documents JSONB
          if (Array.isArray(application.documents)) {
            const enhancedDocs = (application.documents as any[]).map((doc) => {
              // Try matching by filePath first
              const docPath = doc.fileUrl || doc.filePath;
              let verification = docPath ? verificationByPath.get(docPath) : null;
              ...
              // Try matching by id
              const docById = tableDocuments.find((td) => td.id === doc.id);
              if (docById) {
                return {
                  ...doc,
                  verificationStatus: docById.verificationStatus ?? null,
                  verificationNotes: docById.verificationNotes ?? null,
                };
              }

              // Fallback: try matching by documentType
              const docType = doc.documentType;
              const verificationByTypeMatch = docType ? verificationByType.get(docType) : null;
              if (verificationByTypeMatch) {
                return {
                  ...doc,
                  verificationStatus: verificationByTypeMatch.verificationStatus,
                  verificationNotes: verificationByTypeMatch.verificationNotes,
                };
              }
