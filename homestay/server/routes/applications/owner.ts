...
import { deriveDistrictRoutingLabel, isPangiSubDivision } from "@shared/districtRouting";
import {
  MAX_ROOMS_ALLOWED,
  MAX_BEDS_ALLOWED,
...
export function createOwnerApplicationsRouter({ getRoomRateBandsSetting }: OwnerRouterDeps) {
  ...
  router.patch("/:id/draft", requireAuth, async (req, res) => {
    try {
      ...
      const sanitizedDraft = sanitizeDraftForPersistence(validatedData, user, true);
      // console.log("[draft:update] SANITIZED guardianRelation:", sanitizedDraft.guardianRelation);
