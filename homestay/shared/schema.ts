...
// ====================================================================
// Kotak Mahindra (CCAvenue) Transactions
// ====================================================================

export const ccavenueTransactions = pgTable("ccavenue_transactions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  applicationId: varchar("application_id").notNull().references(() => homestayApplications.id),

  // Transaction identifiers
  orderId: varchar("order_id", { length: 50 }).notNull().unique(), // Our unique reference (starts with HPT...)
  trackingId: varchar("tracking_id", { length: 50 }).unique(), // CCAvenue Reference ID
  ...
  // Response Fields
  orderStatus: varchar("order_status", { length: 50 }).default('Initiated'), // Initiated, Success, Failure, Aborted, Invalid
  failureMessage: text("failure_message"),
  paymentMode: varchar("payment_mode", { length: 50 }),
  cardName: varchar("card_name", { length: 50 }),
  statusCode: varchar("status_code", { length: 10 }), // API status code
  statusMessage: text("status_message"),
  bankRefNo: varchar("bank_ref_no", { length: 50 }), // Bank reference number

  // Timestamps
  transDate: timestamp("trans_date"), // Timestamp from CCAvenue
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertCcavenueTransactionSchema = createInsertSchema(ccavenueTransactions)
  .omit({ id: true, createdAt: true, updatedAt: true });

export const selectCcavenueTransactionSchema = createSelectSchema(ccavenueTransactions);
export type InsertCcavenueTransaction = z.infer<typeof insertCcavenueTransactionSchema>;
export type CcavenueTransaction = typeof ccavenueTransactions.$inferSelect;
...
export const ddoCodes = pgTable("ddo_codes", {
  ...
  head1: varchar("head1", { length: 50 }), // Optional Override for Head1 (e.g. 0230-00-104-01)
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
