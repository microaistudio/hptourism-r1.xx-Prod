...
export function Step3RoomsCategory({
    ...
    useEffect(() => {
        if (isDeleteRooms && type2Rows.length > 0) {
            type2Rows.forEach(row => {
                if (initialQuantitiesRef.current[row.id] === undefined) {
                    initialQuantitiesRef.current[row.id] = row.quantity;
                }
            });
        }
    }, [isDeleteRooms, type2Rows]);
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input
                                            ...
                                            className={`h-11 no-spinners ${(!isDeleteRooms && (!field.value || field.value <= 0)) ? "!border-amber-400 !ring-2 !ring-amber-200" : ""} ${isDeleteRooms ? "bg-gray-100 text-gray-500 pointer-events-none" : ""}`}
                                    ...
                                    {!isDeleteRooms && (!field.value || field.value <= 0) && (
                                        <p className="text-xs text-amber-600 font-medium">Required field</p>
                                    )}
                                    <FormMessage />
                                </FormItem>
                            )}
                                        ...
                                        <FormControl>
                                            <div className="relative">
                                                <Input
                                                    ...
                                                    className={`h-11 pr-16 no-spinners ${isFixed ? "bg-gray-50" : ""} ${isDeleteRooms ? "bg-gray-100 text-gray-500" : ""}`}
                                                    readOnly={isFixed || isDeleteRooms}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {type2Rows.map((row, index) => {
                                ...
                                // For delete_rooms, cap the max rooms at the initial loaded quantity
                                const deleteRoomsCap = isDeleteRooms ? (initialQuantitiesRef.current[row.id] ?? row.quantity) : 999;
                                const maxRoomsForRow = Math.max(1, Math.min(remainingRooms, maxRoomsByBeds, deleteRoomsCap));
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="md:col-span-2 min-w-0">
                                            ...
                                            {isDeleteRooms ? (
                                                <div className="h-9 px-3 py-2 text-sm border border-input rounded-md bg-muted text-muted-foreground flex items-center">
                                                    {ROOM_TYPE_OPTIONS.find(opt => opt.value === row.roomType)?.label || row.roomType}
                                                </div>
                                            ) : (
                                                <Select
                                                    value={row.roomType}
                                        ...
                                        <div className="md:col-span-4 grid grid-cols-2 gap-2 min-w-0">
                                            <div>
                                                ...
                                                {isDeleteRooms && fixedQuantities && fixedQuantities[row.roomType as keyof typeof fixedQuantities] !== undefined ? (
                                                    <div className="h-9 px-3 py-2 text-sm border border-input rounded-md bg-muted text-muted-foreground flex items-center font-semibold">
                                                        {fixedQuantities[row.roomType as keyof typeof fixedQuantities]} Rooms
                                                    </div>
                                                ) : (
                                                    <Select
                                                            ...
                                                            <div className="relative">
                                                                ...
                                                                <Input
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        // Use Math.round to prevent floating-point drift (e.g. 2500 becoming 2497)
