...
import {
  CheckCircle2, XCircle, MapPin, Phone, Mail, Bed, IndianRupee, FileText, ArrowLeftCircle,
  CalendarClock, ClipboardCheck,
  RefreshCw, FileImage, Download, Images, Award, CreditCard, QrCode, Printer
} from "lucide-react";
...
export default function ApplicationDetail() {
  ...
  // Reactivate Application Mutation
  const reactivateMutation = useMutation({
    mutationFn: async (reason: string) => {
      const response = await apiRequest("POST", `/api/admin/applications/${applicationId}/reactivate`, {
        reason,
      });
      return response.json();
    },
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The application has been reactivated and moved to submitted status.",
      });
      setReactivateReason("");
    },
    onError: (error: Error) => {
  ...
  const isDistrictOfficer = user?.role === "district_officer"; // Legacy Role
  const isDTDO = user?.role === "district_tourism_officer";
        ...
        <section className="space-y-3 break-inside-avoid">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p>{getDisplayDistrictLabel(app.district)} · {displayValue(app.tehsil)}</p>
            </div>
            <div>
        ...
        <div className="max-w-6xl mx-auto">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="flex items-center gap-2" data-testid="text-district">
                      ...
                      {getDisplayDistrictLabel(app.district)}
                    </p>
                  </div>
                </div>
                <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              ...
              {(isDistrictOfficer || isStateOfficer || isDTDO || isSuperAdmin) && (
                <Card>
                  <CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {currentStatus === 'rejected' && (isSuperAdmin || isDTDO) && (
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button variant="outline" className="w-full text-green-600 hover:text-green-700 border-green-200 hover:bg-green-50" data-testid="button-reactivate">
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>Reactivate Application</DialogTitle>
                            <DialogDescription>
                              This will move the application back to 'Submitted' status and allow one additional correction attempt.
                            </DialogDescription>
                          </DialogHeader>
                          <div className="space-y-4">
                            <div>
                              ...
                              <Textarea
                                id="reactivateReason"
                                placeholder="Explain why this application is being reactivated (e.g., 'Applicant provided clarification via offline channel')..."
                                value={reactivateReason}
                                onChange={(e) => setReactivateReason(e.target.value)}
                                rows={3}
                                data-testid="input-reactivate-reason"
                              />
                            </div>
                          </div>
                          <DialogFooter>
                            <Button
                              ...
                              disabled={reactivateMutation.isPending || !reactivateReason || reactivateReason.length < 5}
