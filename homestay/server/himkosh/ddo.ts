...
export const resolveDistrictDdo = async (district?: string | null, tehsil?: string | null) => {
  if (!district) {
    return undefined;
  }

  // Use the centralized routing logic to get the correct "reporting district" label
  // This handles merged districts (Una -> Hamirpur) and split districts (Pangi, Kaza)
  const routedDistrict = deriveDistrictRoutingLabel(district, tehsil) || district;

  const [exact] = await db
    .select()
    .from(ddoCodes)
    .where(eq(ddoCodes.district, routedDistrict))
    .limit(1);
  ...
  // Fallback: Fuzzy match logic
  const allCodes = await db.select().from(ddoCodes);
  const districtTokens = normalizeDistrictForMatch(routedDistrict);
