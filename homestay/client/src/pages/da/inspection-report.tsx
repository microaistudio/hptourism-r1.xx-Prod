...
export default function DAInspectionReport() {
  ...
  const handleQuickPassCheck = () => {
    // 1. Mandatory Checklist Only (User request: Leave desirable as is)
    handleSelectAllMandatory();

    // 2. Verifications
    form.setValue("roomCountVerified", true, { shouldDirty: true, shouldValidate: true });
    form.setValue("categoryMeetsStandards", true, { shouldDirty: true, shouldValidate: true });
    form.setValue("overallSatisfactory", true, { shouldDirty: true, shouldValidate: true });
    form.setValue("recommendation", "approve", { shouldDirty: true, shouldValidate: true });
    ...
    toast({
      title: "Quick Pass Applied",
      description: "Mandatory checks marked. Please review Desirable checks manually.",
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="sticky top-0 z-10 -mx-6 -mt-6 px-6 py-4 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b flex items-center justify-between shadow-sm">
        <div>
          <Button
            variant="ghost"
        ...
        <div>
          <Button
            type="button"
            variant="outline"
            className="border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900 dark:text-emerald-400"
            onClick={handleQuickPassCheck}
          >
            <CheckCircle className="w-4 h-4 mr-2" />
            Quick Pass: Satisfactory
          </Button>
        </div>
      </div>
