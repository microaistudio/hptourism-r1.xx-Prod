...
export function AppSidebar() {
  ...
  const filteredNavigationSections = useMemo(() => {
    if (legacyRegistrationEnabled || user?.role !== "property_owner") {
      return navigationSections;
    }
    return navigationSections.map((section) => ({
      ...section,
      items: section.items.filter((item) => item.url !== "/existing-owner"),
    }));
  }, [navigationSections, legacyRegistrationEnabled, user?.role]);
  ...
  const isOfficer = user?.role && ['dealing_assistant', 'district_tourism_officer', 'district_officer', 'state_officer', 'admin', 'super_admin', 'system_admin'].includes(user.role);
