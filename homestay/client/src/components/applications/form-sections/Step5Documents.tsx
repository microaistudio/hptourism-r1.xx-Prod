...
export function Step5Documents({
    ...
    // Helper to check if a document section has rejected files
    const hasRejectedFile = (files: UploadedFileMetadata[]) =>
        files.some(f => f.verificationStatus === 'rejected' || f.verificationStatus === 'objection_raised' || f.verificationStatus === 'needs_correction');

    // Helper to check if a document section is fully verified (no editing allowed)
    const isFullyVerified = (files: UploadedFileMetadata[]) =>
                ...
                <CardContent className="p-0">
                    ...
                    {isLegacyRC ? (
                        ...
                        <div className="divide-y">
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <h3 className="font-medium text-gray-900 flex items-center gap-2">
                                            ...
                                            {hasRejectedFile(uploadedDocuments.affidavitSection29) && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-amber-500 text-white animate-pulse">⚠ ACTION REQUIRED</span>
                                            )}
                                            {isFullyVerified(uploadedDocuments.affidavitSection29) && isCorrection && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">✓ Verified</span>
                                            )}
                                        </h3>
                            ...
                            {/* Undertaking Form-C */}
                            <div className={`p-6 ${hasRejectedFile(uploadedDocuments.undertakingFormC) ? 'bg-amber-50 border-l-4 border-amber-400' : isFullyVerified(uploadedDocuments.undertakingFormC) && isCorrection ? 'bg-green-50/50 border-l-4 border-green-300' : ''}`}>
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <h3 className="font-medium text-gray-900 flex items-center gap-2">
                                            Undertaking in Form-C <span className="text-red-500">*</span>
                                            {hasRejectedFile(uploadedDocuments.undertakingFormC) && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-amber-500 text-white animate-pulse">⚠ ACTION REQUIRED</span>
                                            )}
                                            {isFullyVerified(uploadedDocuments.undertakingFormC) && isCorrection && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">✓ Verified</span>
                                            )}
                                        </h3>
                            ...
                            {requiresCommercialUtilityProof && (
                                <>
                                    <div className={`p-6 ${hasRejectedFile(uploadedDocuments.commercialElectricityBill) ? 'bg-amber-50 border-l-4 border-amber-400' : isFullyVerified(uploadedDocuments.commercialElectricityBill) && isCorrection ? 'bg-green-50/50 border-l-4 border-green-300' : ''}`}>
                                        <div className="flex items-start justify-between mb-4">
                                            <div>
                                                <h3 className="font-medium text-gray-900 flex items-center gap-2">
                                                    Proof of Commercial Electricity Bill <span className="text-red-500">*</span>
                                                    {hasRejectedFile(uploadedDocuments.commercialElectricityBill) && (
                                                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-amber-500 text-white animate-pulse">⚠ ACTION REQUIRED</span>
                                                    )}
                                                    {isFullyVerified(uploadedDocuments.commercialElectricityBill) && isCorrection && (
                                                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">✓ Verified</span>
                                                    )}
                                                </h3>
                                    ...
                                    <div className={`p-6 ${hasRejectedFile(uploadedDocuments.commercialWaterBill) ? 'bg-amber-50 border-l-4 border-amber-400' : isFullyVerified(uploadedDocuments.commercialWaterBill) && isCorrection ? 'bg-green-50/50 border-l-4 border-green-300' : ''}`}>
                                        <div className="flex items-start justify-between mb-4">
                                            <div>
                                                <h3 className="font-medium text-gray-900 flex items-center gap-2">
                                                    Proof of Commercial Water Bill <span className="text-red-500">*</span>
                                                    {hasRejectedFile(uploadedDocuments.commercialWaterBill) && (
                                                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-amber-500 text-white animate-pulse">⚠ ACTION REQUIRED</span>
                                                    )}
                                                    {isFullyVerified(uploadedDocuments.commercialWaterBill) && isCorrection && (
                                                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">✓ Verified</span>
                                                    )}
                                                </h3>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <h3 className="font-medium text-gray-900 flex items-center gap-2">
                                                ...
                                                {hasRejectedFile(propertyPhotos) && (
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-amber-500 text-white animate-pulse">⚠ ACTION REQUIRED</span>
                                                )}
                                                {isFullyVerified(propertyPhotos) && isCorrection && (
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">✓ Verified</span>
                                                )}
                                            </h3>
