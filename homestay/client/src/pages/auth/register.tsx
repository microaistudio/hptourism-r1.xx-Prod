...
export default function Register() {
  ...
  // Handle SSO pre-fill
  useEffect(() => {
    const ssoDataStr = sessionStorage.getItem('sso_registration_data');
    if (ssoDataStr) {
      try {
        const ssoData = JSON.parse(ssoDataStr);

        // Split full name into First/Last
        const fullName = ssoData.name || "";
        const nameParts = fullName.trim().split(" ");
        const firstName = nameParts[0] || "";
        const lastName = nameParts.slice(1).join(" ") || "";

        // Pre-fill form
        form.setValue("firstName", filterNameInput(firstName));
        form.setValue("lastName", filterNameInput(lastName));
        form.setValue("mobile", ssoData.mobile || "");
        form.setValue("email", ssoData.email || "");

        // Also pre-fill Aadhaar if available (usually not in basic SSO profile, but if it is)
        // HimAccess might return it?

        toast({
          title: "SSO Verified",
          description: "Your details have been pre-filled from HimAccess.",
        });
      } catch (e) {
