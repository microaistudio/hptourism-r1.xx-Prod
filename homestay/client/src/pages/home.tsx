...
export default function HomePage() {
  ...
  // Fetch live production stats
  const { data: stats = { total: 0, approved: 0, rejected: 0, pending: 0 } } = useQuery({
  ...
  const scenicImages = useMemo(
    () => getHeroImagesForTime(),
    []
  );
  const heroImages = useMemo(
    () => (showCmSlide ? [getCmImage(), ...scenicImages] : scenicImages),
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center gap-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={() => setLocation("/procedure-checklist")}>
                  Procedure & Checklist
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      >
        ...
        <div className={`${showCarousel ? 'relative z-10' : ''} max-w-6xl mx-auto text-center`}>
          ...
          <p className={`text-2xl sm:text-3xl mb-8 max-w-3xl mx-auto leading-relaxed ${showCarousel ? 'text-white/90' : 'text-muted-foreground'}`}>
            Portal for Registration /Renewal of Home Stay Units<br />
            Under Himachal Pradesh Home Stay Rules, 2025
          </p>
      ...
      <section className="py-12 px-4 bg-muted/30" id="stats">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card
              className="shadow-md border group cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02]"
            ...
            >
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-sm font-medium text-muted-foreground">
                  <span className="group-hover:hidden">Total Applications</span>
                  <span className="hidden group-hover:inline text-primary">New Portal Only</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="text-center">
                <p className="text-3xl font-bold group-hover:hidden" data-testid="stat-total">
                  <AnimatedCounter value={stats.total} />
                </p>
                <p className="text-3xl font-bold text-primary hidden group-hover:block">
                  <AnimatedCounter value={stats.realtime?.total ?? 0} />
                </p>
              </CardContent>
            </Card>
            ...
            <Card
              className="shadow-md border group cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02]"
              data-testid="card-approved-applications"
            >
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-sm font-medium text-muted-foreground">
                  <span className="group-hover:hidden">Approved Applications</span>
                  <span className="hidden group-hover:inline text-green-600">New Portal Only</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="text-center">
                <p className="text-3xl font-bold text-green-600 group-hover:hidden" data-testid="stat-approved">
                  <AnimatedCounter value={stats.approved} />
                </p>
                <p className="text-3xl font-bold text-green-600 hidden group-hover:block">
                  <AnimatedCounter value={stats.realtime?.approved ?? 0} />
                </p>
              </CardContent>
            </Card>
            ...
            <Card
              className="shadow-md border group cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02]"
              data-testid="card-rejected-applications"
            >
              <CardHeader className="text-center pb-2">
                <div className="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-2 group-hover:bg-red-100 transition-colors">
                  <TrendingUp className="w-6 h-6 text-red-600" />
                </div>
                <CardTitle className="text-sm font-medium text-muted-foreground">
                  <span className="group-hover:hidden">Rejected Applications</span>
                  <span className="hidden group-hover:inline text-red-600">New Portal Only</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="text-center">
                <p className="text-3xl font-bold text-red-600 group-hover:hidden" data-testid="stat-rejected">
                  <AnimatedCounter value={stats.rejected} />
                </p>
                <p className="text-3xl font-bold text-red-600 hidden group-hover:block">
                  <AnimatedCounter value={stats.realtime?.rejected ?? 0} />
                </p>
              </CardContent>
            </Card>
            ...
            >
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-sm font-medium text-muted-foreground">
                  <span className="group-hover:hidden">Pending Applications</span>
                  <span className="hidden group-hover:inline text-orange-600">New Portal Only</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="text-center">
                <p className="text-3xl font-bold text-orange-600 group-hover:hidden" data-testid="stat-pending">
                  <AnimatedCounter value={stats.pending} />
                </p>
                <p className="text-3xl font-bold text-orange-600 hidden group-hover:block">
                  <AnimatedCounter value={stats.realtime?.pending ?? 0} />
                </p>
              </CardContent>
            </Card>
          </div>
          <p className="text-center text-xs text-muted-foreground mt-4">
            Hover on cards to see new portal stats only
          </p>
        </div>
      </section>
