...
export default function DAInspections() {
  ...
  // Report submitted but awaiting final decision (NOT in terminal state)
  const reportAwaitingDecision = inspections?.filter(i => {
    const appStatus = i.application?.status?.toLowerCase() || '';
    return i.reportSubmitted && !terminalStatuses.includes(appStatus);
  }) || [];
  // Completed: report submitted AND application in terminal state, with time filter
  const completedFinal = useMemo(() => {
    const now = new Date();
    return (inspections ?? []).filter(i => {
      const appStatus = i.application?.status?.toLowerCase() || '';
      if (!i.reportSubmitted || !terminalStatuses.includes(appStatus)) return false;
      // Apply time range filter
      const updatedAt = new Date(i.inspectionDate);
  ...
  }, [inspections, completedRange, terminalStatuses]);
  ...
  }, [inspections, activeFilter, pendingInspections, reportAwaitingDecision, completedFinal, newInspectionIds]);
  ...
  const filterLabels: Record<typeof activeFilter, string> = {
    ...
    reportSubmitted: "Awaiting decision",
    completed: "Completed",
    new: "New assignments",
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        {/* Card 3: Report Submitted - Awaiting Decision */}
        <Card
          role="button"
          tabIndex={0}
          aria-pressed={activeFilter === 'reportSubmitted'}
          onClick={() => setActiveFilter('reportSubmitted')}
          onKeyDown={(event) => {
            if (event.key === 'Enter' || event.key === ' ') {
              event.preventDefault();
              setActiveFilter('reportSubmitted');
            }
          }}
          className={`cursor-pointer transition-all hover-elevate active-elevate-2 ${activeFilter === 'reportSubmitted' ? "ring-2 ring-primary" : ""
            }`}
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Report Submitted</CardTitle>
            <ClipboardCheck className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{reportAwaitingDecision.length}</div>
            <p className="text-xs text-muted-foreground mt-1">Awaiting final decision</p>
          </CardContent>
        </Card>

        {/* Card 4: Completed - Time-filtered with outcome badges */}
        <Card
          role="button"
          tabIndex={0}
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{completedFinal.length}</div>
                      ...
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {/* Outcome badge for completed inspections */}
                            {activeFilter === 'completed' && inspection.application?.status && (() => {
                              const appStatus = inspection.application.status.toLowerCase();
                              if (appStatus === 'approved' || appStatus === 'certificate_issued') {
                                return (
                              ...
                              } else if (appStatus === 'superseded' || appStatus === 'certificate_cancelled') {
                                return (
