...
export function generateCertificatePDF(
  ...
  // Chrome-compatible download method using blob and anchor click
  try {
    ...
    // Fallback to standard save if blob method fails
...
function renderPolicyCertificate(
  ...
  doc.setFont("times", "normal");
  doc.text(valueOrBlank(getDisplayDistrictLabel(application.district)), pageWidth - layout.marginX, signatureY + 14, { align: "right" });
...
function resolveTouristArea(application: HomestayApplication) {
  const parts = [
    ...
    getDisplayDistrictLabel(application.district),
  ...
  return getDisplayDistrictLabel(application.district) || "";
}
