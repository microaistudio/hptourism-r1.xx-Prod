...
export function createAdminNotificationsRouter() {
  ...
  router.get("/notifications", requireRole("admin", "super_admin", "system_admin"), async (_req, res) => {
    try {
  ...
  router.put("/notifications", requireRole("admin", "super_admin", "system_admin"), async (req, res) => {
    try {
