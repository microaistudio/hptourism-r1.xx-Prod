  ...
  const completedInspections = useMemo(
    () => {
      ...
      return (inspections ?? []).filter((order) => {
        ...
        if (seen.has(key)) return false;
        seen.add(key);
        // Only show if report is submitted AND application is NOT in a terminal state
        const appStatus = order.application?.status?.toLowerCase() || '';
        return order.reportSubmitted && !terminalStatuses.includes(appStatus);
      });
    },
    [inspections],
  );
            ...
            >
              ...
              <div className="mt-2 flex flex-col gap-0.5 text-[11px]">
                {stage.pills.map((pill) => {
                  ...
                  const isInfoOnly = stage.key === "completed";
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        // Background logic matching DTDO dashboard
                        isActionable || (isInfoOnly && pill.count > 0)
                          ? stage.key === "corrections" || stage.key === "inspections"
